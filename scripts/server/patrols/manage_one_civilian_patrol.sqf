...
while { GRLIB_endgame == 0 } do {
    ...
    _usable_sectors = [];
    {
        if ((([markerPos _x, 1000, GRLIB_side_friendly] call KPLIB_fnc_getUnitsCount) == 0) && (count ([markerPos _x, 3500] call KPLIB_fnc_getNearbyPlayers) > 0)) then {
            _hostile_sector_chance = 10;
            if([ markerpos _x ] call KPLIB_fnc_getSectorOwnership == GRLIB_side_friendly || random 100 <= _hostile_sector_chance) then {
    ...
    if ( count _usable_sectors > 0 ) then {
        ...
        _grp = createGroup [GRLIB_side_resistance, true];
        if ( random 100 < 33) then {
            ...
            _civveh addEventHandler ["HandleDamage", { private [ "_damage" ]; if (( side (_this select 3) != GRLIB_side_friendly ) && ( side (_this select 3) != GRLIB_side_enemy ) && ( side (_this select 3) != GRLIB_side_resistance )) then { _damage = 0 } else { _damage = _this select 2 }; _damage } ];
        ...
        { _x addEventHandler ["HandleDamage", { private [ "_damage" ]; if (( side (_this select 3) != GRLIB_side_friendly ) && ( side (_this select 3) != GRLIB_side_enemy ) && ( side (_this select 3) != GRLIB_side_resistance )) then { _damage = 0 } else { _damage = _this select 2 }; _damage } ]; } foreach units _grp;
